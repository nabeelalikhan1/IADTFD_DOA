...
% %%% DOA Estimation
 ...
 [D_avg,DD,~]=SADTFD_new_iterative(X,2,12,48);
perc=0.4;
 thr = perc*max(max(D_avg));
 Tr = (D_avg) >= thr;
 [F_trace, ~] = find(Tr);
...
% 
% %%% DOA Estimation
 P_tf_music_SADTFD_iterative(iii,:)  = tf_music(D_s, n_sources, N_sensors, 2,1, theta1);
end
P_CKD=mean(P_CKD);
...
P_tf_music_SADTFD_iterative=mean(P_tf_music_SADTFD_iterative);
P_tf_music_SADTFD_iterative=P_tf_music_SADTFD_iterative/max(P_tf_music_SADTFD_iterative);
...
hold on;plot(theta1,log10(P_spatial),'r','linewidth',3);
 ...
 hold on;plot(theta1,log10(P_tf_music_SADTFD_iterative),'g-.','linewidth',3);
 hold on;stem(theta1,A);
xlabel('Spatial Spectrum','FontSize',24);
ylabel('Amplitude (dB)','FontSize',24);
legend('STFD based TF MUSIC','Averaged STFD based TF MUSIC','Both averaged and directionally smoothed STFD','iterative STFD');
